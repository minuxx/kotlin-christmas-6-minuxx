...
이벤트 플래너 프로그램의 핵심 구현 사항은 고객이 방문날짜와 주문 메뉴를 입력하면 주문 내역과 이벤트 혜택 내역을 알려주는 것이다.
...
### Mdoel
#### EventDate
- ✅ 유효한 날짜가 아니면 예외가 발생한다.
  - 12.1 ~ 12.31 사이의 날짜가 아닐 때
...
- ✅ 주문한 메뉴의 총개수가 20개가 초과하면 예외가 발생한다.
...
#### PresentationEvent
- ✅ 혜택 금액을 계산한다.
- ✅ 총주문 금액이 유효하지 않으면 예외가 발생한다.
  - 총주문 금액 최소 120,000원 미만일 때
- ✅ 증정 할인 이벤트날짜가 유효하지 않으면 예외가 발생한다.
  - 12.1 ~ 12.31 사이의 날짜가 아닐 때
